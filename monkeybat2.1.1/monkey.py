...
MONKEY_CMD = 'adb shell monkey --throttle 500 -s 1000 --pct-anyevent 0 --pct-trackball 0 --pct-nav 0 --pct-majornav 2 --pct-appswitch 2 \
--ignore-timeouts --ignore-crashes --ignore-security-exceptions  --pkg-blacklist-file /data/local/tmp/WhiteBlack.txt -v -v  576000 > '
...
m1=re.match('OK',LIST_PACKAGE,re.I)
if m1:
    pass
else:
    print 'Input error'
...
CHOICE = raw_input()
if CHOICE == str('z') or CHOICE == str('Z'):
    os.system(PUSH_WHITHBLACK)
    subprocess.Popen(monkeyCmd('zj'),stdout=subprocess.PIPE,shell=True)
    print 'Start full test' 
elif CHOICE == str('m') or CHOICE == str('M'):
    os.system(PUSH_WHITHBLACK)
...
while 1:
	try:		
		...
		if INPUT_interrupt == str('q') or INPUT_interrupt == 'Q':
			break
