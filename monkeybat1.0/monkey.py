...
def Dumpkmsg():
    filepath = os.path.join(path_time, 'kmsg.txt')
    file_handler=open(filepath,'w+')
    file_handler.write(q)    
    file_handler.close()
def kill_logcat():
...
print '*'*70
print ' package list已经生成到package.txt\n 如果进行白名单测试请将想要测试的package复制到.\l\WhiteBlack.txt'
print ' 如果进行整机测试,请将避免测试的package复制到.\l\WhiteBlack.txt;WhiteBlack.txt也可以为空.\n monkey可以通过参数自动识别黑白名单'
print ' WhiteBlack.txt编辑完成后,请输入 ok\n'
print '*'*70
LIST_PACKAGE = raw_input() 
if LIST_PACKAGE == str('ok'):
    print 'WhiteBlack.txt is ok'
else:
    print 'Input error'
    sys.exit()
print '='*70
print ' Press \"z\" to start whole test of Monkey;\n Press \"m\" to start list test of Monkey:\n'
print '='*70

CHOICE = raw_input()
if CHOICE == str('z'):
	os.system('start /MIN  .\l\zjmonkey.bat')
	print '开始整机测试' 
elif CHOICE == str('m'):
	os.system('start /MIN  .\l\mdmonkey.bat')
	print '开始名单测试'
else:
	print 'Input Error'
	sys.exit()
...
path_time = os.path.join(cur_file_dir(),otherStyleTime,'test')
#print path_time
make_sure_path_exists(path_time)

sysstr = platform.system()
if(sysstr =="Windows"):
    os.system('start /MIN logcatsplit.bat')
    os.system('start /MIN .\l\kmsg.bat')
   
elif(sysstr == "Linux"):
    os.system('sh logcatsplit.sh')
    os.system('sh kmsg.sh')
time.sleep(10)  
