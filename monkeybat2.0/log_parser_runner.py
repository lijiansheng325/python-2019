...
ADB_FETCH_LOGS_RESUME_CMD = 'adb logcat -v threadtime'
...
ANR_CLEAN_CMD = "adb shell rm -rf /data/anr"
SDCARD_CLEAN_SEMS_CMD = "adb shell rm -rf /sdcard/sems "
SDCARD_CLEAN_LOGS_CMD = "adb shell rm -rf /sdcard/logs "
DROPBOX_CLEAN_CMD = "adb shell rm -rf /data/system/dropbox "
TOMBSTONES_CLEAN_CMD = "adb shell rm -rf /data/tombstones "
...
def clean_device_log():
    ...
    call_adb_clean_log(SDCARD_CLEAN_SEMS_CMD)
    call_adb_clean_log(SDCARD_CLEAN_LOGS_CMD)
...
def usage():  
    ...
    -s : Parser step
    Example:
    log_parser_runner -a 5 -b 5 -c 1 -o 'Report_Folder' -s 10000
    '''
...
try:
    opts, args = getopt.getopt(sys.argv[1:],'a:b:c:o:s:')
...
if len(opts) == 0:  
    ...
    try:
        ...
        REPORT_NAME = opts[3][1]
        PARSER_STEP = int(opts[4][1])
    except IndexError:
        usage()  
        sys.exit()    
...
#test_name = os.path.join(REPORTS_PATH, otherStyleTime)
#test_name = REPORTS_PATH + str(otherStyleTime)
test_name = os.path.join(REPORTS_PATH, REPORT_NAME)
